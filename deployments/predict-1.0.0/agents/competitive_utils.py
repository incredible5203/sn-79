...
def ascend_score_tick(
    ...
    *,
    ...
    base_qty = sim_order_qty(min_quantity, max_quantity, quantity_scale, vdec)
    completion_qty = sim_order_qty(min_quantity, min_quantity, 1.0, vdec)

    def _quote_qty(book_id: int) -> float:
        if book_quote_qty and book_id in book_quote_qty:
    ...
    for _prio, book_id, best_bid, best_ask, _mid, trade_dir, touch_comp in requote_jobs:
        ...
        account = accounts[book_id]
        if trade_dir == OrderDirection.BUY:
            if price >= best_ask or account.quote_balance.free < completion_qty * price:
                continue
        elif price <= best_bid or account.base_balance.free < completion_qty:
            continue
        if not budget.can_place(book_id, 1):
            continue
        place_limit(
            response, book_id, trade_dir, completion_qty, price, expiry_period,
