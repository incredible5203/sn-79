...
class PredictiveMakerAgent(FinanceSimulationAgent):
    ...
    def _place_completions(
        ...
        for book_id, hint in list(self._completions.items()):
            ...
            if hint.side == "BUY":
                ...
                price = self._round_price(max(target, bid_p), pdec)
                if price >= ask_p:
                    price = self._round_price(ask_p - tick, pdec)
            else:
                trade_dir = OrderDirection.SELL
                target = self._round_price(hint.fill_price + edge, pdec)
                price = self._round_price(min(target, ask_p), pdec)
                if price <= bid_p:
                    price = self._round_price(bid_p + tick, pdec)
    ...
    def _flatten_inventory(
        ...
        for book_id, account in self.accounts.items():
            ...
            if qty < self.min_quantity:
                continue
            response.limit_order(book_id, trade_dir, qty, price)
            budget.use(book_id)
